...
output: 
  flexdashboard::flex_dashboard:
    ...
    theme:
      ...
      navbar-bg: "#DFDFE8"  #Color barra de navegación
...
caption_icono_tooltip <- function(
  caption,
  tooltip_text = "Info"
) {

  icon_svg <- '
    <svg id="info-icon" style="cursor:pointer;vertical-align:middle"
      ...
      <path d="m8.93 6.588-2.29.287-.082.38.45.083
               c.294.07.352.176.288.469L6.684 10.5
               c-.194.897.105 1.319.808 1.319
               .545 0 1.178-.252 1.465-.598l.088-.416
               c-.2.176-.492.246-.686.246
               -.275 0-.375-.193-.304-.533z"/>
      <circle cx="8" cy="4.5" r="1"/>
    </svg>
  '

  caption_html <- paste0(
    '<div style="display:flex;align-items:center;gap:4px;font-size:11px;line-height:16px;min-height:16px">',
    '<span>', caption, '</span>',
    icon_svg,
    '</div>'
  )
  ...
  list(
    ...
    tooltip_js = JS(sprintf("
      function () {

        const chart = this;

        // buscar el icono SOLO dentro del gráfico
        const info = chart.renderTo.querySelector('#info-icon');

        if (!info) {
          console.warn('Icono info-icon no encontrado');
          return;
        }
        ...
        info.addEventListener('mouseover', function () {
          ...
          if (!chart.infoTooltip) {
            chart.infoTooltip = chart.renderer
              .label('%s', 10, 10)
              .attr({
                zIndex: 12,
                fill: '#fff',
          ...
          const iconBox = info.getBoundingClientRect();
          const chartBox = chart.renderTo.getBoundingClientRect();

          const x = iconBox.right - chartBox.left + 8;
          const y = iconBox.top - chartBox.top - 4;
...
cap_c1 <- caption_icono_tooltip(
  caption = "",
  tooltip_text = "Para hacer zoom, sombrear las barras dentro del gráfico de forma vertical. <br> 
  Luego puede desplazarse usando el scrollbar."
)
...
layout_column_wrap(
  ...
  card_body(
   contador_area %>% 
    ...
    hc_title(text = "Avance de Respuestas por División") %>%
    # hc_subtitle(text = "Frecuencias por Nivel de Respuesta") %>%
    hc_chart(events = list(load = cap_c1$tooltip_js))%>%
    hc_caption(text = cap_c1$caption_html, useHTML = TRUE, style=list(fontSize=16)) %>% 
    hc_xAxis(title = list(text = "")) %>% 
...
contador_area2 <- principal %>% 
  filter(!is.na(area2)) %>% # Evitar vacios
  group_by(area2,estado) %>% 
  summarise(respuestas = n()) %>% 
  mutate(prop = round(respuestas/sum(respuestas)*100,0)) %>%
  pivot_wider(names_from = estado, values_from = c(respuestas,prop), values_fill = 0) %>% 
  mutate(total_respuestas = rowSums(across(starts_with("respuestas"))),
...
layout_column_wrap(
  ...
  card_body(
   contador_area2 %>% 
    hchart("bar", hcaes(x = area2, y = total_prop, z = total_respuestas), name = "Población", showInLegend = TRUE,
           borderRadius = 10,
           dataLabels=list(enabled=TRUE, format="{point.z:.0f}")) %>%
    hc_add_series(type = "bar",data = contador_area2,hcaes(x = area2, y = prop_Completado, z = respuestas_Completado),
                  name = "Completado", color = "#29BEB4",showInLegend = TRUE,
                  borderRadius = 10,
                  dataLabels=list(enabled=TRUE, format="<span style='color:{series.color}'>{point.z:.0f} ({point.y:.0f}%)</span>")) %>%
